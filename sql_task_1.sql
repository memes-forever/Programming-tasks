...
test_data_with_window AS (
    SELECT
        ...
        end as g_lag -- проставляем первому попавшему диапазону единицу
    FROM test_data
),
main as (
    select
        test_data_with_window.*,
        sum(g_lag) over (order by id) as g_sum -- суммируем единицы по группам, тем саммым получаем уникальные группы диапазонов
